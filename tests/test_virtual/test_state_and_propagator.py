...
def test_state_manipulate_herm_time_dep():
    ...
    vprop.propagate(vstate, t=8, tlist=[0, 4, 8])
...
def test_state_manipulate_nonherm_time_dep():
    ...
    vprop.propagate(vstate, t=0.2)
    assert isclose(fidelity(vstate.qobj, Qobj([[0.34553939+0.j, -0.00580815+0.02835213j],
...
def test_state_jumps():
    ...
    vprop.propagate(vstate, t=2)
    assert isclose(fidelity(vstate.qobj, vstate.qobj), 0.6228716916889016)
